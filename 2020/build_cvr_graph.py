...
def clear_graph_race(db, county_id, races):
    ...
    try:
        for race in races:
            ...
            cur.execute("DELETE FROM cvr_2020_graph WHERE county_id=%s AND race_id=%s", (county_id, race[0]))
            db.commit()
    finally:
        cur.close()
...
def has_data(db, county_id, race_id):
    cur = db.cursor()
    try:
        cur.execute("SELECT COUNT(1) FROM cvr_2020_graph WHERE county_id=%s AND race_id=%s", (county_id, race_id))
...
def build_race(db, county_id, races):
    ...
    try:
        for race in races:
            ...
            cur.execute(f"INSERT INTO cvr_2020_graph (county_id, race_id, record, candidate) SELECT %s, %s, record, {race[2]} FROM cvr_2020_{county_id}_raw AS c1 WHERE {race[2]} != '' ORDER BY record", (county_id, race_id))
            db.commit()

        totals = {}
        cur.execute("SELECT record, candidate FROM cvr_2020_graph WHERE county_id=%s AND race_id=%s ORDER BY record", (county_id, race_id))
        rows = cur.fetchall()
        vcount = 0
        for row in rows:
            record = row[0]
            ...
            cur.execute("UPDATE cvr_2020_graph SET vcount=%s, ccount=%s WHERE county_id=%s AND race_id=%s AND candidate=%s AND record=%s", (vcount, ccount, county_id, race_id, candidate, record))
            db.commit()
    finally:
        cur.close()
...
def build_race_stats(db, county_id):
    ...
    try:
        ...
        cur.execute("INSERT INTO race_stats SELECT county_id, race_id, MAX(vcount) FROM cvr_2020_graph WHERE county_id=%s AND vcount IS NOT NULL GROUP BY county_id, race_id", (county_id))
        db.commit()
    finally:
        cur.close()
...
def print_summary(db, county_id):
    ...
    cur = db.cursor()
    try:
        cur.execute(f"SELECT DISTINCT race, candidate FROM cvr_2020_{county_id}_graph ORDER BY race, candidate")
        for row in cur:
            race, candidate = row
