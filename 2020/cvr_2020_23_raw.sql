START TRANSACTION;
DROP TABLE IF EXISTS `cvr_2020_23_raw`;
CREATE TABLE `cvr_2020_23_raw` (
  ...
  `county_council` tinytext NOT NULL,
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='Cast vote record 2020 Wicomico County';
...
INSERT INTO `county_race` (`county_id`, `race_id`, `raw_column`)
SELECT @county_id, id, "county_council" FROM `race` WHERE name="County Council";
...
INSERT INTO `county_race` (`county_id`, `race_id`, `raw_column`)
SELECT @county_id, id, "board_of_education" FROM `race` WHERE name="Board of Education";
