START TRANSACTION;
DROP TABLE IF EXISTS `cvr_2020_1_raw`;
CREATE TABLE `cvr_2020_1_raw` (
  ...
  `city_council_of_cumberland_1` tinytext NOT NULL,
  `city_council_of_cumberland_2` tinytext NOT NULL,
  `q1` tinytext NOT NULL,
  `q2` tinytext NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='Cast vote record 2020 Allegany County';
...
INSERT INTO `county_race` (`county_id`, `race_id`, `raw_column`)
SELECT @county_id, id, "city_council_of_cumberland_1" FROM `race` WHERE name="City Council of Cumberland";

INSERT INTO `county_race` (`county_id`, `race_id`, `raw_column`)
SELECT @county_id, id, "city_council_of_cumberland_2" FROM `race` WHERE name="City Council of Cumberland";
